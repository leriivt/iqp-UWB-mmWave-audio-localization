...
class Uart_ui(object):
    def __init__(self, frame_uart, frame_trx):
        ...
        tk.Label(frame_uart, text="端口", bg="lightblue").place(x=10, y=10)
        self.this_com_combobox = ttk.Combobox(frame_uart, values="", width=9)
        self.this_com_combobox.place(x=50, y=10)

        tk.Label(frame_uart, text="波特率", bg="lightblue").place(x=150, y=10)
        ...
        self.this_open_button = ttk.Button(frame_uart, text="打开")
        self.this_open_button.place(x=300, y=7)

        self.this_uart_com_update_button = ttk.Button(frame_uart, text="更新", command=lambda:uart_module.serial_update(self.this_com_combobox))
