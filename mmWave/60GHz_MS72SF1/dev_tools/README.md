...
2. You will need to install some sort of serial terminal software to send serial commands to the MS72SF1 to configure it. The Development Guide uses SSCOM which is found at www.daxia.com.
   > - When using SSCOM, you must change your system language for non-Unicode programs to **Chinese**:
   >    - `Control Panel` -> `Clock & Region` -> `Region` -> `Administrative` -> `Language for non-Unicode programs`.
   > - Select the **HEXSHOW** checkbox in SSCOM to make the data intelligible.
