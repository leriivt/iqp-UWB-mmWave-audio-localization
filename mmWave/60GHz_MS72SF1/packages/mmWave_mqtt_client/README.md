...
Client-side MQTT handling for mmWave measurements (RPi).

## Overview

This package provides MQTT communication for mmWave localization systems with two main components:

- **`client.py`**: MQTT client that handles network communication and publishing
- **`mmWave_hardware.py`**: Hardware interface that reads from mmWave serial devices (UART) and processes measurements
...
- Reads UART data from mmWave modules (preserves exact parsing logic)
